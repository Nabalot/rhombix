...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public boolean create(LookAhead myObj) {

        boolean createSuccessful = false;

        if(!checkIfExists(myObj.name)){

            SQLiteDatabase db = this.getWritableDatabase();

            ContentValues values = new ContentValues();
            values.put(fieldObjectName, myObj.name);
    ...
    public List<LookAhead> read(String searchTerm) {

        List<LookAhead> recordsList = new ArrayList<LookAhead>();
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                LookAhead myObject = new LookAhead(objectName);
